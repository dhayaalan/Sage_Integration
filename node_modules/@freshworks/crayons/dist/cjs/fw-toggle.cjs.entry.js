...
const Toggle = class {
  ...
  render() {
    const toggleSize = ['small', 'medium', 'large'].includes(this.size)
      ? this.size
      : 'medium';
    return (index.h(index.Host, { onClick: () => this.toggle(), tabindex: '0', role: 'switch', "aria-disabled": this.disabled ? 'true' : 'false', "aria-checked": this.checked ? 'true' : 'false', "aria-label": this.label }, index.h("div", { class: {
        'toggle-switch': true,
        [toggleSize]: true,
      } }, index.h("input", { name: this.name, type: 'checkbox', disabled: this.disabled, checked: this.checked, class: 'checkboxClass' }), index.h("span", { class: {
        slider: true,
        [toggleSize]: true,
      } }, index.h("span", { class: 'before' }, this.showIcon && (index.h("fw-icon", { color: this.checked ? '#2c5cc5' : '#647a8e', name: this.checked ? 'check' : 'cross', class: { checked: this.checked }, library: 'system' })))))));
  }
